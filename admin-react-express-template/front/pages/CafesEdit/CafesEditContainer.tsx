...
const CafesEditContainer = (props: any) => {
  const navigate = useNavigate();
  const { cafeNumber } = props;
  ...
  const onSubmit = useCallback((value, actions) => {
    ...
    /*  이미지 리스트 추출 */
    const imageLists = cafeImageState.images.map(v => {
      return {
        dataURL: v.dataURL.indexOf('s3.ap-northeast-2.amazonaws.com') !== -1 ? v.dataURL : null,
        fileName: v.fileName,
        index: v.index,
        rank: v.rank,
        file: isEmpty(v.file) ? {} :
    ...
    formData.append('imageLists', JSON.stringify(imageLists));

    if (!isEmpty(cafeImageState.images)) {
      [].forEach.call(cafeImageState.images, (f: any) => {
        if (!isEmpty(f.file)) formData.append('image', f.file);
      });
    }

    return axios.post(`/api/cafes/${cafeNumber}`, formData, { withCredentials: true })
      .then(result => {
        alert('수정완료');
        actions.setSubmitting(false);
  ...
  return (isEmpty(cafeState?.cafeInfo?.cafeTitle) ?
    <Stack direction="row" spacing={0} style={{ alignItems: 'center', justifyContent: "center", height: '90%' }}>
...
export default memo(CafesEditContainer);

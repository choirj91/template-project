...
const LatestOrders = (props: any) => {
  ...
  const { cafeLists, revalidate } = props;
  const navigate = useNavigate();

  const [isDisplaying, setIsDisplaying] = useState(false);
  const [cafes, setCafes] = useState(cafeLists);
          ...
          <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle" size={'small'}>
            <TableHead>
              <TableRow>
                ...
                <TableCell align={'center'} padding={'none'}>노출</TableCell>
...
export default memo(LatestOrders);
